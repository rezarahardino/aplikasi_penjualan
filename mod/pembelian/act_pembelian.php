...
<?php
	...
	elseif ($mod == "pembelian" AND $act == "editqty") {
		...
		if (count($rowNums) > 0) {
			...
			foreach ($rowNums as $i) {
				...
				mysql_query("UPDATE tb_detail_pembelian SET harga_beli = '".$_POST['harga_beli_'.$i]."', 
															qty = '".$_POST['qty_'.$i]."' 
							WHERE kode_barang = '".$_POST['kode_'.$i]."' 
							AND no_faktur = '$_POST[no_faktur]'") or die(mysql_error());
					
			}
			
		}
			
		flash('example_message', '<p>Berhasil mengedit data transaksi pembelian.</p>' );
		echo"<script>
			window.history.back();
		</script>";	
	}
	...
	elseif ($mod == "pembelian" AND $act == "batal") {
		...
		flash('example_message', '<p>Berhasil membatalkan data transaksi pembelian.</p>' );
		echo"<script>
			window.history.back();
		</script>";	
	}
