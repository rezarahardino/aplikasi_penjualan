<?php
	...
	switch ($act) {
				...
				<div style='border-bottom:1px dashed #ccc;'></div><br>";
					echo"<table class='w3-table w3-tiny w3-hoverable tbl'>
						...
						echo"<tr class='w3-light-grey'>
							...
							<a href='$aksi?mod=penjualan&act=bataltransaksi' class='w3-text-red' onclick=\"return confirm('Yakin ingin membatalkan semua transaksi?');\">Hapus Semua</a></center>
							</td>
						</tr>
						</tbody>
			...
			<div id="id01" class="w3-modal">
			  <div class="w3-modal-content">
			    <div class="w3-container">
			      ...
			      <h3>Edit Qty</h3>
			      <form action="<?php echo $aksi; ?>?mod=penjualan&act=editqty" method="POST">
			      <?php
			      echo"<table class='w3-table w3-tiny w3-hoverable tbl'>
						<thead>
						<tr class='w3-blue'>
						...
						if(mysql_num_rows($sql_tmp) > 0)
						{
							...
							while ($b = mysql_fetch_assoc($sql_tmp)) {
								...
								<tr style='border-bottom:1px dashed #ccc;'>
									<td><input type='hidden' name='rowNums[]' value='$i' />
		...
		case "printout" :
			...
			if(isset($_GET['id']))
			{
				...
				echo"<h4 style='float:right'>Detail Barang
					...
					<a href='kwitansi.php?id=$_GET[id]' class='w3-btn w3-dark-grey w3-tiny' target='_blank'><i class='fa fa-print'></i> Cetak PDF</a>
				    </h4>
		...
		case "edit" :
			...
			if(isset($_GET['id']))
			{
				if (!isset($_GET['sts']) OR $_GET['sts'] <> 'ok') {
					mysql_query("DELETE FROM tb_detail_penjualan_edit_tmp WHERE petugas = '$_SESSION[login_id]'") or die(mysql_error());
					$rsqledit = mysql_query("SELECT * FROM tb_detail_penjualan WHERE no_transaksi = '$_GET[id]'") or die(mysql_error());
					while ($d = mysql_fetch_assoc($rsqledit)) {
						mysql_query("INSERT INTO tb_detail_penjualan_edit_tmp(kode_barang, 
																			harga,
																			disc, 
																			qty, 
																			petugas)
																	VALUES('$d[kode_barang]', 
																			'$d[harga]', 
																			'$d[disc]', 
																			'$d[qty]', 
																			'$_SESSION[login_id]')") or die(mysql_error());
					}
				}
				...
				echo"<form method='POST' action='$aksi?mod=penjualan&act=edit' class='w3-container'>
				<div class='w3-col s8 w3-padding'>
					...
					$sql = mysql_query("SELECT a.*, b.nama_barang, b.satuan 
										FROM tb_detail_penjualan_edit_tmp a LEFT JOIN tb_barang b 
										ON a.kode_barang = b.kode_barang
										WHERE a.petugas = '$_SESSION[login_id]'") or die(mysql_error());
					$sub_total = 0;
					$total = 0;
					$no = 1;
					while($p = mysql_fetch_assoc($sql))
					{
						...
						echo"<tr>
							...
							<td>Rp. ".number_format($p['harga'])." X $p[qty]</td>
							<td><center>".number_format($p['disc'])."%</center></td>
					...
					$total_harga = $total - $tra['potongan'];
					$sisa = $tra['bayar'] - $total_harga;
					...
					echo"</tbody>
						...
						<tr class='w3-light-grey'>
							<td colspan='6'><center>
							<a href='#' class='w3-text-blue' onclick=\"document.getElementById('id01').style.display='block'\">Edit Transaksi</a></center>
							</td>
						</tr>
				...
				<div class='w3-col s4'>
					<div class='w3-card w3-padding w3-light-blue'>
						...
						<input type='hidden' name='no_transaksi' id='no_transaksi' value='".$tra['no_transaksi']."'>
						<input type='hidden' name='totalbyr' id='totalbyr'>
						<input type='hidden' name='jmlbayar2' id='bayar2'>
						...
						<label class='w3-label w3-text-black'>Potongan (Rp):</label>
						<input type='text' name='potongan' id='potongan' class='w3-input w3-border w3-tiny' value='0'>
						
						<label class='w3-label w3-text-black'>Status Pembayaran:</label>
						<div class='w3-row'>
							<div class='w3-col s6'>
								<input type='radio' class='w3-radio' name='status' value='LUNAS' "?><?php echo (isset($tra['status']) AND $tra['status'] == 'LUNAS') ? 'checked' : ''; ?><?php echo">
								<label class='w3-validate'>LUNAS</label>
							</div>
							<div class='w3-col s6'>
								<input type='radio' class='w3-radio' name='status' value='HUTANG' "?><?php echo (isset($tra['status']) AND $tra['status'] == 'HUTANG') ? 'checked' : ''; ?><?php echo">
								<label class='w3-validate'>HUTANG</label>
							</div>
						</div>
			...
			<div id="id01" class="w3-modal">
			  <div class="w3-modal-content">
			    <div class="w3-container">
			      ...
			      <h3>Edit Qty</h3>
			      <form action="<?php echo $aksi; ?>?mod=penjualan&act=editqty1" method="POST">
			      <input type='hidden' name='no_transaksi' id='no_transaksi' value='<?php echo $tra['no_transaksi']; ?>'>
			      <?php
			      echo"<table class='w3-table w3-tiny w3-hoverable tbl'>
						<thead>
						<tr class='w3-blue'>
						...
						$sql_tmp = mysql_query("SELECT a.kode_barang, a.qty, a.harga, b.nama_barang, a.disc 
												FROM tb_detail_penjualan_edit_tmp a, tb_barang b
												WHERE a.kode_barang = b.kode_barang 
												AND a.petugas = '$_SESSION[login_id]'") or die(mysql_error());
						$no = 1;
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						...
						echo"<tr>
							...
							<a class='w3-text-blue w3-hover-text-blue' href='med.php?mod=penjualan&act=edit&id=$m[no_transaksi]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a>
							<!--<a href='$aksi?mod=penjualan&act=editqty&id=$m[no_transaksi]' class='w3-text-blue' onclick=\"document.getElementById('id01').style.display='block'\">Edit Transaksi</a>
							<a href='$aksi?mod=penjualan&act=editqty&id=$m[no_transaksi]';\"><i class='fa fa-pencil-square w3-large w3-text-red'></i></a>-->
							</td>
