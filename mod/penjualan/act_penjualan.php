...
<?php
	...
	elseif ($mod == "penjualan" AND $act == "bataltransaksi") {
		mysql_query("DELETE FROM tb_detail_penjualan_tmp 
	...
	elseif ($mod == "penjualan" AND $act == "editqty") {
	...
	elseif ($mod == "penjualan" AND $act == "editqty1") {
		...
		if (count($rowNums) > 0) {
			...
			foreach ($rowNums as $i) {
				...
				if (mysql_num_rows($cek_barang) > 0) {
					...
					$sisa = (($total_stok + $retur_jual) - $retur_beli);
					if ($sisa >= $_POST['qty_'.$i]) {
						mysql_query("UPDATE tb_detail_penjualan_edit_tmp SET qty = '".$_POST['qty_'.$i]."',  
																		disc = '".$_POST['disc_'.$i]."' 
									WHERE kode_barang = '".$_POST['kode_'.$i]."' 
									AND petugas = '$_SESSION[login_id]'") or die(mysql_error());
		...
		header("location:../../med.php?mod=penjualan&act=edit&id=".$no_transaksi."&sts=ok");
	}

	if ($mod == "penjualan" AND $act == "edit") 
	{
		$qtmp = mysql_query("SELECT * FROM tb_detail_penjualan_edit_tmp 
							WHERE petugas = '$_SESSION[login_id]'");

		if (mysql_num_rows($qtmp) > 0) {
			$no_transaksi = $_POST['no_transaksi']; //no transaksi automatis
			...
			if (($total_bayar <= $jmlbayar) OR ($_POST['status'] == "HUTANG")) {
				...
				//pembuatan header
				$qsimpanheader = mysql_query("UPDATE tb_penjualan SET status = '$_POST[status]',
																		bayar = '$jmlbayar', 
																		potongan = '$_POST[potongan2]' 
				...
				{
					mysql_query("DELETE FROM tb_detail_penjualan WHERE no_transaksi = '$_POST[no_transaksi]'") or die(mysql_error());
		...
		if (mysql_num_rows($qtmp) > 0) {
			...
			if (($total_bayar <= $jmlbayar) OR ($_POST['status'] == "HUTANG")) {
				...
				else
				{
					...
					mysql_query("DELETE FROM tb_detail_penjualan_tmp WHERE petugas = '$_SESSION[login_id]'") or die(mysql_error());
