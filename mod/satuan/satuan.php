<?php
	...
	$aksi = 'mod/satuan/act_satuan.php';

	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				$act = "$aksi?mod=satuan&act=edit";
				...
				{
					header("location:med.php?mod=satuan");
				}

			}
			else
			{
				$act = "$aksi?mod=satuan&act=simpan";
			}
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mod' value='satuan'>
							<div class='w3-row'>
								<div class='w3-col s1'>
					...
					<td align='right'><a href='med.php?mod=satuan' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=satuan&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$m[nama_satuan]</td>
							<td>$m[tgl_input]</td>
							<td><a href='med.php?mod=satuan&act=form&id=$m[id_satuan]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mod=satuan&act=hapus&id=$m[id_satuan]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
